...
# %%
amip_index = area_weighted_ave(cmip_concat['ua'].sel(lat=slice(-10,10)).sel(time=slice('1979-01-01','2014-12-31')).squeeze())

# %%
std = area_weighted_ave(cmip_concat['ua'].sel(lat=slice(-10,10))).std('time') #.plot()
std
# %%
cmip_concat.where(std == std.max(),drop=True).member_id.values
# %%
p = amip_index.plot(col='member_id',col_wrap=4,x='time',c='tab:orange',alpha=0.5,add_legend=False)
for ax in p.axes.flat:
    era5
# %%
ds_x = era5.resample(time='1MS').mean('time').sel(time=slice('1979-01-01','2014-12-31'))
ds_x['time'] = np.arange('1979-01-01','2015-01-01',dtype='datetime64[M]')
ds_y = amip_index.sel(time=slice('1979-01-01','2014-12-31'))
ds_y['time'] = np.arange('1979-01-01','2015-01-01',dtype='datetime64[M]')

r = linregress(ds_y,ds_x, dim='time')
r
# %%
for i in sorted(r.r.values):
    print(i)

# %%
r_sorted = sorted(r.r.values)
# %%
r.where(r.r == r_sorted[-3:],drop=True) #['member_id']

# %%
r_sorted.member_id.values

# %%
amp_sorted = sorted(amip_max-amip_min)
# %%
r.where((amip_max-amip_min) == amp_sorted[-4],drop=True) #['member_id']

# %%
r.r.sel(member_id='GISS-E2-2-G_r1i1p1f1').values #.plot()
# %%
amip_std.sortby(amip_std,ascending=False) #.plot()
# %%
r.r.sortby(r.r,ascending=False) #.plot()
# %%
amip_rmac = amip_ua.groupby('time.month') - amip_ua.groupby('time.month').mean('time')
# %%
amip_amp = 0.5 * amip_rmac.std('time')
amip_amp = amip_amp.sortby(amip_amp,ascending=False) #.plot()
amip_amp
# %%
r = r.sortby(r.r,ascending=False)
r
# %%
