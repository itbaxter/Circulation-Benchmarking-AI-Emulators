...
import xarray as xr
#---ERA5
dir = '/scratch/midway3/itbaxter/NeuralGCM_Decadal_Simulations/scripts/Randel_Held_1991/u_trend/'
...
files = sorted(glob.glob('/scratch/midway2/itbaxter/NeuralGCM_Decadal_Simulations/scripts/circulation_variability/wavenumber_frequency/ace2/ace2_*_u250.nc'))
print(files)

u_ace2 = xr.open_dataset(files[6])['eastward_wind_2'] #ACE_READER(files[10],'eastward_wind_2').read().compute()
u_ace2

# %%
files = sorted(glob.glob('/scratch/midway2/itbaxter/NeuralGCM_Decadal_Simulations/scripts/circulation_variability/wavenumber_frequency/ace2/ace2_*_v250.nc'))
print(files)

v_ace2 = xr.open_dataset(files[6])['northward_wind_2'] #ACE_READER(files[10],'eastward_wind_2').read().compute()
v_ace2


# %%
files = sorted(glob.glob('/scratch/midway3/itbaxter/NeuralGCM_Decadal_Simulations/scripts/Randel_Held_1991/u_trend/u250/*ngcm*nc'))
files

u_ngcm = xr.open_dataset(files[20])['u_component_of_wind'].resample(time='1D').mean('time') #.sel(level=250) #.mean('latitude')
u_ngcm

# %%
files = sorted(glob.glob('/scratch/midway3/itbaxter/NeuralGCM_Decadal_Simulations/scripts/Randel_Held_1991/u_trend/v250/*ngcm*nc'))
print(files)

v_ngcm = xr.open_dataset(files[20])['v_component_of_wind'].resample(time='1D').mean('time') #.sel(level=250) #.mean('latitude')
v_ngcm

# %%
file = '/project/tas1/itbaxter/for-tiffany/amip/121x240/ua/ua_CMIP6_CESM2-WACCM_day_amip_1950-2016.nc'

u_amip = xr.open_dataset(file).sel(plev=25000)['ua'].squeeze()
u_amip

# %%
file = '/project/tas1/itbaxter/for-tiffany/amip/121x240/va/va_CMIP6_CESM2-WACCM_day_amip_1950-2016.nc'
...
# %%
years = np.arange(1961, 2002, 1)
p_spec, c = process(u_era5, v_era5, years, season='DJFM', apply_window=True)
...
# %%
C = c[0, :]
dc = xr.DataArray(np.gradient(C), coords={'c': C}, dims=['c'])
#C = C[C != 0]
p_spec.coords['c'] = C
coslat = (np.cos(np.radians(u_era5['lat'])))

R = 6.371e6
dx = 2 * np.pi * R / float(128)  # meters per longitude grid point
dx_scaled = dx * np.cos(np.radians(u_era5['lat'])) #/ float(j)

data = (p_spec.sel(year=slice(1981,1987)).mean('year')/dc).where(abs(p_spec['c']) >= 1)
...
contours = data.plot.contour(x='c', levels=p.levels, linewidths=0.6, colors='k')

plt.clabel(contours, inline=True, fontsize=8, fmt='%1.1f')

get_winds(u_era5.mean('lon'), season='DJFM').squeeze().plot(y='lat',c='k')
...
# %%
coslat = np.cos(np.deg2rad(u_era5['lat']))
emfc_era5 = compute_emfc_clean(1000 * p_spec) #, lat_name='lat')
emfc_era5

# %%
r = linregress(emfc_era5.sel(year=slice(1961,2000)), emfc_era5.sel(year=slice(1961,2000)).year,dim='year')
r
...
#(p_spec.mean('year')/dc/coslat).where(abs(p_spec['c']) >= 1).plot.contourf(x='c',levels=np.arange(-0.10,0.101,0.02), cmap='RdBu_r')
data = (1000*p_spec.sel(year=slice(1981,1987)).mean('year')/dc).where(abs(p_spec['c']) >= 1)
p = data.plot.contourf(
    #levels=np.arange(-3.0,3.01,0.5), 
    levels=levels,
    #levels=21,
...
contours = data.plot.contour(x='c', levels=[l for l in p.levels if l != 0], linewidths=0.6, colors='k')

plt.clabel(contours, inline=True, fontsize=8, fmt='%1.1f')

get_winds(u_era5.mean('lon'), season='DJFM').squeeze().plot(y='lat',c='k')
plt.axvline(0, color='k', linestyle='--', linewidth=0.5)
ax.set_title('UV Power Spectrum (DJFM): RH91')

ax = fig.add_subplot(212)
ax.set_xlim([-10, 40])

lat_range = (-85, -12)  # Adjusted latitude range for plotting

coslat = np.cos(np.radians(u_era5['lat']))
levels = [-0.1,-0.08,-0.06,-0.04,-0.02,0.02,0.04,0.06,0.08,0.1]
p = (emfc_era5.mean('year') / dc).sel(lat=slice(*lat_range)).where(abs(emfc_era5['c']) >= 1).plot.contourf(x='c', 
    levels=21, 
    extend='both', cmap='RdBu_r')
contours = (emfc_era5.mean('year')).sel(lat=slice(*lat_range)).where(abs(emfc_era5['c']) >= 1).plot.contour(x='c', 
    levels=p.levels, linewidths=0.6, colors='k')

c1 = (40*r.slope.where(r.slope > 0) / dc).sel(lat=slice(*lat_range)).plot.contour(x='c', y='lat', 
    levels=3, 
    #levels = np.arange(0.01, 0.081, 0.01), 
    colors='tab:red', linewidths=1.0)
c2 = (40*r.slope.where(r.slope < 0) / dc).sel(lat=slice(*lat_range)).plot.contour(x='c', y='lat', 
    levels = 3, 
    #levels = np.arange(-0.08,-0.011, 0.01), 
    colors='tab:blue', linewidths=1.0)

plt.clabel(c1, inline=True, fontsize=8,)# fmt='%1.2f')
plt.clabel(c2, inline=True, fontsize=8,) # fmt='%1.2f'

(get_winds(u_era5.sel(time=slice('1961-01-01','2001-12-31')).mean('lon'), season='DJFM')/coslat).sel(lat=slice(*lat_range)).squeeze().plot(y='lat',c='k')

ax.set_title('EMFC (DJFM): CH07')

plt.tight_layout()
plt.savefig('./emfc_era5_djfm.png', dpi=300, bbox_inches='tight')
gc.collect()
# %%
gc.collect()

# %%
(emfc_era5.sel(year=slice(1961,2001)).mean('year')).sel(lat=slice(-85,-5)).plot()
(get_winds(u_era5.mean('lon'), season='DJFM')/coslat).sel(lat=slice(-85,-5)).squeeze().plot(y='lat',c='k')
...
plt.savefig('/scratch/midway2/itbaxter/NeuralGCM_Decadal_Simulations/plots/Figure_2-RH91-v2.png', dpi=300)
# %%
...
plt.savefig('/scratch/midway2/itbaxter/NeuralGCM_Decadal_Simulations/plots/Figure_2-CH07-v2.png', dpi=300)
# %%
