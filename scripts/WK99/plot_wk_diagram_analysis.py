...
def plot_normalized_symmetric_spectrum(s, ax, panel, label='ERA5', ylabel='None', ofil=None, cbar=False):
    ...
    add_tropical_wave_annotations(ax, component='sym') #, alpha=0.15, text_size=8, show_labels=True)
...
if __name__ == "__main__":
    ...
    era5.coords['component'] = ['symmetric','antisymmetric']
    ...
    symComponent = xr.open_mfdataset(fili,combine='nested',concat_dim='member_id')['symComponent']    # Load 
    ...
    ngcm = xr.open_dataset('./data/ngcm_Components.nc')['__xarray_dataarray_variable__']


    ngcm_background = xr.open_mfdataset(sorted(glob.glob(f'./data/ngcm_background_*.nc')),combine='nested',concat_dim='member_id')['__xarray_dataarray_variable__']
    fili = sorted(glob.glob(f"./data/amip*nc"))

    def preprocess(ds):
        return ds.assign_coords(component=0)
    amip = xr.open_mfdataset(fili,combine='nested',concat_dim='component',preprocess=preprocess)['__xarray_dataarray_variable__']
    amip.coords['component'] = ['antisymmetric','background','symmetric']
