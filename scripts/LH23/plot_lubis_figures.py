...
# Uncomment to run:
fig = create_lubis_four_panel_simple()
plt.show(), gc.collect()
...
# %%
era5_cross = compute_modes_v2(era5_new)
era5_cross

# %%
ace2_cross = [compute_modes_v2(ace2_new.sel(member_id=i)) for i in ace2_new.member_id]
ace2_cross = xr.concat(ace2_cross, dim='member_id')
ace2_cross

# %%
ngcm_cross = [compute_modes_v2(ngcm_new.sel(member_id=i)) for i in ngcm_new.member_id]
ngcm_cross = xr.concat(ngcm_cross, dim='member_id')
ngcm_cross

# %%
amip_cross = [compute_modes_v2(amip_new.sel(member_id=i)) for i in amip_new.member_id]
...
# %%
fig = plt.figure(figsize=(7.5,9))
gs = GridSpec(1,1,figure=fig,hspace=0.3, wspace=0.25,top=0.94,bottom=0.05, left=0.08, right=0.98)
ax = fig.add_subplot(gs[0])

for i in range(len(amip_member_ids)):
    amip['z2z1'].where(amip['z1z1'].mean('lag') <= 0.9).sel(lag=slice(-lag,lag)).isel(member_id=i).plot.line(x='lag',c='silver',add_legend=False)

cmip['z2z1'].mean('member_id').sel(lag=slice(-lag,lag)).plot(c='tab:purple',linewidth=1.5,label='CMIP')

amip['z2z1'].where(amip['z1z1'].mean('lag') <= 0.9).mean('member_id').sel(lag=slice(-lag,lag)).plot(c='tab:orange',linewidth=1.5,label='AMIP')
#ace2_train['z2z1'].mean('member_id').sel(lag=slice(-lag,lag)).plot(c='tab:green',linewidth=1.5,label='ACE2-ERA5 [2011-2017]')

#erai['z2z1'].sel(lag=slice(-lag,lag)).plot(c='k',linewidth=1.5,linestyle='--',label='ERA-Interim')
era5_cross['z2z1'].sel(lag=slice(-lag,lag)).plot(c='k',linewidth=1.5,label='ERA5')
ax.set_title(r'AMIP $\mathrm{z_{2}z_{1}}$')
ax.set_xlabel('')
ax.axhline(0,linestyle='--',c='k',linewidth=0.7)
ax.axvline(0,linestyle='--',c='k',linewidth=0.7)
...
p = era5['r'].plot.contourf(yincrease=False,robust=True,cmap='RdBu_r',extend='neither',levels=np.arange(-1.0,1.1,0.1),add_colorbar=False)
c = era5['r'].plot.contour(yincrease=False,robust=True,colors='k',extend='neither',linewidths=0.7,levels=np.arange(-1.0,1.1,0.1),add_colorbar=False)

ax.text(0.0, 1.15, 'a', transform=ax.transAxes, fontsize=16, fontweight='bold', va='top')
ax.set_ylim([-25,-80])
ax.set_title('ERA5',weight='bold')
plt.colorbar(p)

ax = fig.add_subplot(gs[0,1])

p = amip['r'].where(amip['z1z1'].mean('lag') <= 0.9).mean('member_id').plot.contourf(yincrease=False,robust=True,cmap='RdBu_r',extend='neither',levels=np.arange(-1.0,1.1,0.1),add_colorbar=False)
c = amip['r'].where(amip['z1z1'].mean('lag') <= 0.9).mean('member_id').plot.contour(yincrease=False,robust=True,colors='k',extend='neither',linewidths=0.7,levels=np.arange(-1.0,1.1,0.1),add_colorbar=False)

ax.text(0.0, 1.15, 'b', transform=ax.transAxes, fontsize=16, fontweight='bold', va='top')
ax.set_ylim([-25,-80])
ax.set_title('AMIP',weight='bold')
plt.colorbar(p)

ax = fig.add_subplot(gs[1,0])

p = ace2['r'].mean('member_id').plot.contourf(yincrease=False,robust=True,cmap='RdBu_r',extend='neither',levels=np.arange(-1.0,1.1,0.1),add_colorbar=False)
c = ace2['r'].mean('member_id').plot.contour(yincrease=False,robust=True,colors='k',extend='neither',linewidths=0.7,levels=np.arange(-1.0,1.1,0.1),add_colorbar=False)

ax.text(0.0, 1.15, 'c', transform=ax.transAxes, fontsize=16, fontweight='bold', va='top')
ax.set_ylim([-25,-80])
ax.set_title('ACE2-ERA5',weight='bold')
plt.colorbar(p)
plt.tight_layout()
...
ax.set_ylim([-25,-80])
ax.set_title('NeuralGCM',weight='bold')
plt.colorbar(p)
plt.tight_layout()
